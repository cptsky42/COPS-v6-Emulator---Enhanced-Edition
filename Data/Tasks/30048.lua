...
function processTask30048(client, idx)
    name = "StageGuard"
    face = 1

    if (idx == 0) then

        if hasItem(client, 721106, 1) then
    ...
    elseif (idx == 1) then
        ...
        if hasTaskItem(client, "PassTokenL100") and (getMoney(client) >= 0) then
