...
function processTask3118(client, idx)
    name = "HighCabinet"
    face = 1
    ...
    if (idx == 0) then
        ...
        if (getMoney(client) >= 0) and (getPkPoints(client) >= 0 and getPkPoints(client) <= 1000) then
            ...
            text(client, "Do you want this HighCabinet? It costs only 8000 silvers.")
            link(client, "Yes.", 1)
            link(client, "No.", 255)
            pic(client, 188)
            create(client)
        ...
        if (getMoney(client) >= 0) and (getPkPoints(client) >= 0 and getPkPoints(client) <= 1000) then
            ...
            if getMoney(client) < 8000 then
                ...
                if (getItemsCount(client) <= 39) then
                    ...
                    awardItem(client, "721183", 1)
                    sendSysMsg(client, "You got a HighCabinet", 2005)
