...
function processTask30047(client, idx)
    name = "StageGuard"
    face = 1

    if (idx == 0) then

        if hasItem(client, 721105, 1) then
    ...
    elseif (idx == 1) then
        ...
        if hasTaskItem(client, "PassTokenL90") and (getMoney(client) >= 0) then
