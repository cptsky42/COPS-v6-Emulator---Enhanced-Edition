...
function processTask3124(client, idx)
    name = "ItemBox"
    face = 1
    ...
    if (idx == 0) then
        ...
        if (getMoney(client) >= 0) and (getPkPoints(client) >= 0 and getPkPoints(client) <= 1000) then
            ...
            text(client, "Do you want this ItemBox? It is expensive, and costs 100000 silvers.")
            link(client, "Yes.", 1)
            link(client, "No.", 255)
            pic(client, 188)
            create(client)
    ...
    elseif (idx == 1) then
        ...
        if (getMoney(client) >= 0) and (getPkPoints(client) >= 0 and getPkPoints(client) <= 1000) then
            ...
            if getMoney(client) < 100000 then
                ...
                text(client, "You do not have 100000 silvers with you.")
                ...
                if (getItemsCount(client) <= 39) then
                    ...
                    awardItem(client, "721189", 1)
                    sendSysMsg(client, "You got an ItemBox", 2005)
