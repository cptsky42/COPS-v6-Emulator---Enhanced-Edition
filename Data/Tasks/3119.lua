...
function processTask3119(client, idx)
    name = "FoldingScreen"
    face = 1
    ...
    if (idx == 0) then
        ...
        if (getMoney(client) >= 0) and (getPkPoints(client) >= 0 and getPkPoints(client) <= 1000) then
            ...
            text(client, "Do you want this FoldingScreen? It costs only 12000 silvers.")
            link(client, "Yes.", 1)
            link(client, "No.", 255)
            pic(client, 188)
            create(client)
        ...
        if (getMoney(client) >= 0) and (getPkPoints(client) >= 0 and getPkPoints(client) <= 1000) then
            ...
            if getMoney(client) < 12000 then
                ...
                if (getItemsCount(client) <= 39) then
                    ...
                    awardItem(client, "721184", 1)
                    sendSysMsg(client, "You got a FoldingScreen", 2005)
